...
class Crc_tratativas extends Model
{
    ...
    public function listTipoContrato($cod = null)
    {
        ...
        $sql = "SELECT * FROM public.crc_tratativa_tipo where cod_tipo_tratativa not in (6) order by tipo_tratativa asc";
        $sql = $this->db->prepare($sql);

        $parametro = [];
        if ($cod) {
            $sql .= " WHERE cod_tipo_tratativa = :cod";
            $parametro[':cod'] = $cod;
        }
    ...
    public function listTipoAcoes($cod = null)
    {
        ...
        $sql = "SELECT * FROM public.crc_tipo_acoes  where cod_acao not in (6) order by acao_descricao asc";
        $sql = $this->db->prepare($sql);

        $parametro = [];
        if ($cod) {
            $sql .= " WHERE cod_acao = :cod";
            $parametro[':cod'] = $cod;
        }
    ...
    public function getRelatorio($mes = null, $data_inicio = null, $data_fim = null)
    {
        ...
        $sql .= " GROUP BY 
                ...
                st.cod_status
                        ORDER BY 
                            ultima_info DESC,
                st.crc_tratativas_id DESC limit 2";
    ...
    public function insertMovimentacao($idCobranca, $dados = null)
    {
        ...
        try {
            ...
            $new_ocorrencia =  'Foi inserido a movimentacao do tipo ' . $new_tipo['tipo_tratativa'] . ' com a ação ' . $crc_tipo_acoes_id . ' para a cobrança ' . $idCobranca;
