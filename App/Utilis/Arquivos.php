...
class Arquivos {
    ...
    if (!empty($r['resultado'])) {
        foreach ($r['resultado'] as $values) {
            ...
            //   echo $camposAquisicao . $jsonResposta;
            echo "<br>***<br>";
        }
    }
	...
	// list($camposAquisicao, $jsonResposta) = [ $r['campo_aquisicao'], $r['resposta_json'] ];
		...
		if ($jsonObjProscore->registro) {
			...
			foreach ($registros as $plugin => $arrayValues) {
				...
				if ($configPlugin->separar) { // SAIDA ARQUIVO A PARTE - GRAVA CADA LINHA
					...
					// $confCns = MontaJsonConfigEHeadersDaConsulta::execute($r['codcns']);
					...
					foreach ($linhasSaidaVertical as $linhaSaidaVertical) { // PARA CADA OCORRENCIA do PLUGIN
						...
						if ($linhaSaidaVertical!="") {
							...
							// GravaRespostaPlugin::execute($plugin, $camposAquisicao.";".$linhaSaidaVertical, $r['transacao_id'], $header);
						}
					}
	...
	if (!$success) { // saida de ERRO
		...
		// GravaTransacao::execute($r['processo_id'], $camposAquisicao, 3, 0, null, null);
		...
		if ($linhaSaidaHorizontal!="") {
			...
			// GRAVA RESPOSTA TRANSACAO SUCESSO
			// GravaTransacao::execute($r['processo_id'], $camposAquisicao, 3, 1, $linhaSaidaHorizontal, $jsonResposta);
		}
	}
